    ...
    <div id="page-content" class="clearfix">
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <!-- 检索  -->
                <form action="line/list.do" method="post" name="Form" id="Form">
...
<script type="text/javascript">
    ...
    function add(PARENT_ID) {
        ...
        diag.Drag = true;
        diag.Title = "新增";
        diag.URL = '<%=basePath%>/line/goAdd.do?PARENT_ID=' + PARENT_ID;
        diag.Width = 400;
        diag.Height = 355;
    ...
    function del(Id) {
        bootbox.confirm("确定要删除吗?", function (result) {
            if (result) {
                var url = "<%=basePath%>/line/delete.do?LINE_ID=" + Id + "&tm=" + new Date().getTime();
    ...
    function edit(Id) {
        ...
        diag.Drag = true;
        diag.Title = "编辑";
        diag.URL = '<%=basePath%>/line/goEdit.do?LINE_ID=' + Id;
        diag.Width = 400;
        diag.Height = 355;
        diag.CancelEvent = function () { //关闭事件
            if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
...
<script type="text/javascript">
            ...
            if (result) {
                ...
                if (str == '') {
                    ...
                    if (msg == '确定要删除选中的数据吗?') {
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>/line/deleteAll.do?tm=' + new Date().getTime(),
                            data: {DATA_IDS: str},
                            dataType: 'json',
    ...
    //导出excel
    function toExcel() {
        window.location.href = '<%=basePath%>/line/excel.do';
    }
</script>
