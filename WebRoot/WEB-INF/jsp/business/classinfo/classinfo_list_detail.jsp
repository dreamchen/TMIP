    ...
    <div id="page-content" class="clearfix">
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <!-- 检索  -->
                <form action="classinfo/list.do" method="post" name="Form" id="Form">
                    <table>
                        <tr>
                            <td>
                    ...
                    <table id="table_report" class="table table-striped table-bordered table-hover">
                        ...
                        <tr>
                            ...
                            <th class="center">班主任老师</th>
                            <th class="center">营养师</th>
                            <th class="center">班级类型</th>
                            <th class="center">班级状态</th>
                            <th class="center">开班时间</th>
                            <th class="center">成长阶段开始时间</th>
                            <th class="center">成长阶段结束</th>
                            <th class="center">状态改变日期</th>
                            <th class="center">班级简介</th>
                                        ...
                                        <tr>
                                            <td class='center' style="width: 30px;">
                                                <label><input type='checkbox' name='ids'
                                                              value="${pd.CLASSINFO_ID}"/><span
                                                        class="lbl"></span></label>
                                            </td>
                                            <td class='center' style="width: 30px;">1</td>
                                            <td>${pd.KINDERGARTEN_ID}</td>
                                            <td>${pd.CLASS_NAME}</td>
                                            <td>${pd.QUANTITY}</td>
                                            <td>${pd.MAIN_TEACHER_ID}</td>
                                            <td>${pd.DIETITIAN_ID}</td>
                                            <td>${pd.CLASS_TYPE}</td>
                                            <td>${pd.STATUS}</td>
                                            <td>${pd.CLASS_BEGIN_TIME}</td>
                                            <td>${pd.GROWTH_STAGE_BEGIN}</td>
                                            <td>${pd.GROWTH_STAGE_END}</td>
                                            <td>${pd.STATUS_CHANGE_TIME}</td>
                    ...
                    <div class="page-header position-relative">
                        <table style="width:100%;">
                            <tr>
                                ...
                                -->
                                <a class="btn btn-small btn-success" onclick="history.go(-1);">返回</a>
                                <td style="vertical-align:top;">
...
<script type="text/javascript">
    ...
    function add() {
        ...
        diag.Drag = true;
        diag.Title = "新增";
        diag.URL = '<%=basePath%>classinfo/goAdd.do';
        diag.Width = 450;
        diag.Height = 355;
    ...
    function del(Id) {
        bootbox.confirm("确定要删除吗?", function (result) {
            if (result) {
                top.jzts();
                var url = "<%=basePath%>classinfo/delete.do?CLASSINFO_ID=" + Id + "&tm=" + new Date().getTime();
    ...
    function edit(Id) {
        ...
        diag.Drag = true;
        diag.Title = "编辑";
        diag.URL = '<%=basePath%>classinfo/goEdit.do?CLASSINFO_ID=' + Id;
        diag.Width = 450;
        diag.Height = 355;
        diag.CancelEvent = function () { //关闭事件
            if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
...
<script type="text/javascript">
            ...
            if (result) {
                ...
                if (str == '') {
                    ...
                    if (msg == '确定要删除选中的数据吗?') {
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>classinfo/deleteAll.do?tm=' + new Date().getTime(),
                            data: {DATA_IDS: str},
                            dataType: 'json',
    ...
    //导出excel
    function toExcel() {
        window.location.href = '<%=basePath%>classinfo/excel.do';
    }
</script>
