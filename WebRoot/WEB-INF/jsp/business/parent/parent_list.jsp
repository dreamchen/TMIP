    ...
    <div id="page-content" class="clearfix">
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <!-- 检索  -->
                <form action="parent/list.do" method="post" name="Form" id="Form">
                    <table>
                        <tr>
                            <td>
                    ...
                    <table id="table_report" class="table table-striped table-bordered table-hover">
                        ...
                        <tr>
                            ...
                            <th class="center">被监护人</th>
                            <th class="center">亲属关系</th>
                                ...
                                <c:if test="${QX.cha == 1 }">
                                    <c:forEach items="${varList}" var="var" varStatus="vs">
                                        <tr>
                                            ...
                                            <td>${var.RELATION}</td>
                                            <td>${var.BLOOD_TYPE}</td>
                                            <td>${var.GENDER}</td>
                                            <td>${var.ID_CARD_NUM}</td>
                                            <td>${var.TELEPHONE}</td>
                                            <td>${var.REMARK}</td>
                    ...
                    <div class="page-header position-relative">
                        <table style="width:100%;">
                            <tr>
                                <td style="vertical-align:top;">
                                    <c:if test="${QX.add == 1 }">
                                    	<!-- 如果有权限，并且按学生查看时候才能添加，如果只是浏览所有的监护人，那不能添加 -->
                                    	<c:if test="${not empty pd.STUDENT_ID }">
...
<script type="text/javascript">
    ...
    function add() {
        ...
        diag.Drag = true;
        diag.Title = "新增";
        diag.URL = '<%=basePath%>parent/goAdd.do?param=xx&param1=xb&STUDENT_ID='+ "${pd.STUDENT_ID}";
        diag.Width = 450;
        diag.Height = 355;
    ...
    function del(Id) {
        bootbox.confirm("确定要删除吗?", function (result) {
            if (result) {
                top.jzts();
                var url = "<%=basePath%>parent/delete.do?PARENT_ID=" + Id + "&tm=" + new Date().getTime();
    ...
    function edit(Id) {
        ...
        diag.Drag = true;
        diag.Title = "编辑";
        diag.URL = '<%=basePath%>parent/goEdit.do?PARENT_ID=' + Id+"&param=xx&param1=xb";
        diag.Width = 450;
        diag.Height = 355;
        diag.CancelEvent = function () { //关闭事件
        	if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
...
<script type="text/javascript">
            ...
            if (result) {
                ...
                if (str == '') {
                    ...
                    if (msg == '确定要删除选中的数据吗?') {
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>parent/deleteAll.do?tm=' + new Date().getTime(),
                            data: {DATA_IDS: str},
                            dataType: 'json',
    ...
    //导出excel
    function toExcel() {
        window.location.href = '<%=basePath%>parent/excel.do';
    }
</script>
